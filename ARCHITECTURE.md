...
| 层       | 选型                                            | 原因                                        |
| -------- | ----------------------------------------------- | ------------------------------------------- |
| 小程序   | Taro 4.2.0 + React 18 + TypeScript              | 延续当前 React 心智，同时输出微信小程序代码 |
...
```text
users
  id, wechatOpenId, nickname, avatarFileId?,
...
产品指标的名称、分子分母、去重、近似边界和报告方式统一由 [METRICS.md](./METRICS.md) 定义。MVP 在 M9-04 复用业务数据与低成本首次成功标记生成 Markdown/CSV 报告；M10-01 再增加建议评价记录和评价指标，不提前建设独立事件平台或运营后台。

不得记录请求正文中的 Token、完整位置和图片 URL。
...
- 天气服务采用和风天气，开发期使用免费订阅，接入前确认配额与许可。
- LLM 服务采用腾讯混元，模型和预算在实现任务前固定。
- 抠图采用腾讯云媒体处理 MPS，先以免费额度或小样确认质量、费用和图片保留策略，不开启自动后付费。
...
| 项目         | 当前值                                     | 状态与说明                                                 |
| ------------ | ------------------------------------------ | ---------------------------------------------------------- |
| 本地 Node.js | 20.19.2                                    | 与 CloudBase `Nodejs20.19` 运行时对齐                      |
| CloudBase    | 上海地域 `ootd-ai-dev`                     | 整个 MVP 共用，不创建其他环境                              |
| 小程序样板   | Taro 4.2.0、React 18.2.0、TypeScript 5.8.x | 独立 `miniprogram/` 包已完成本地构建，待开发者工具重新验收 |
| 微信基础库   | 3.16.2                                     | 已由项目所有者确认，不长期跟随“最新”                       |

CloudBase 当前支持 `Nodejs20.19`、`Nodejs18.15` 和 `Nodejs16.13`，本项目选择 `Nodejs20.19`。[CloudBase 运行时说明](https://docs.cloudbase.net/en/cloud-function/develop/introduce)

### 13.2 版本决策

- 生产小程序固定使用 Taro 4.2.0、React 18.2.0 和 TypeScript 5.8.x。
- 小程序依赖放在独立的 `miniprogram/` 包中，根项目仅通过 `npm --prefix miniprogram run build:weapp` 调用构建。
- 本地微信小程序构建已通过；完成开发者工具编译、预览和真机打开后关闭本决策门。

### 13.3 外部服务候选

| 能力 | 首选候选                   | MVP 使用建议                                                                                                     | 状态   |
| ---- | -------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------ |
| 天气 | 和风天气                   | 开发期使用个人开发者免费订阅（1000 次/天），生产前核对署名与许可要求；服务端使用独立 API Host 和密钥             | 已确认 |
| LLM  | 腾讯混元                   | 云函数服务端调用，4 秒超时后使用本地规则；模型与预算在 M5-02 前固定                                              | 已确认 |
| 抠图 | 腾讯云媒体处理 MPS AI 抠图 | 使用 `ProcessImage`，场景 `ScheduleId=30031`；M6-03 前以免费额度或衣物小样验证质量、耗时和费用，不开启自动后付费 | 已确认 |

参考：

- [和风天气订阅与免费额度](https://dev.qweather.com/docs/finance/subscription/)
- [和风天气 API Host 与认证](https://dev.qweather.com/docs/configuration/api-host/)
- [腾讯混元对话 API](https://cloud.tencent.com/document/product/1729/105701)
- [腾讯云媒体处理 AI 抠图](https://cloud.tencent.com/document/product/862/132593)

### 13.4 验收结果

- Taro 4.2.0 产物已在微信开发者工具完成编译。
- 代码质量扫描“启用组件按需注入”已通过。
- 预览和真机打开已由项目所有者确认成功。
