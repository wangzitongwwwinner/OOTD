...
- Taro 4.2.0、React 18、TypeScript 5.x。
...
- 外部服务适配器：天气、LLM、图片抠图。供应商由环境配置决定，领域层不依赖供应商 SDK。
...
- 游客示例数据与用户业务数据必须使用不同的数据源和命名空间；游客草稿不得写入用户缓存或云端集合。
- 受保护操作通过统一登录门控保存待执行意图；登录成功后幂等续接一次，取消、失败或重复回调不得产生写入。
- 用户首次登录只初始化示例场景和示例行程，必须可重入且幂等；重试只补齐缺失副本、不覆盖已有修改，用户删除后不得再次补回。示例衣物和默认搭配不得写入账号。
