...
function mockRepository(
  findByDate: ItineraryRepository["findByDate"],
): ItineraryRepository {
  return {
    findByDate,
    create: async () => item,
...
test("查询当天行程返回成功信封", async () => {
  ...
  const repo = mockRepository(async (d, id) => {
    receivedDate = d;
    receivedIdentity = id;
    return [item];
  });
  const now = new Date("2026-07-20T10:00:00.000Z");
  const res = await getTodayItineraries(
    { openId: "o", appId: "a" },
    repo,
    "req1",
    () => now,
...
test("空行程返回空列表", async () => {
  const repo = mockRepository(async () => []);
  const now = new Date("2026-07-20T10:00:00.000Z");
  const res = await getTodayItineraries(
    { openId: "o", appId: "a" },
    repo,
